...
[설치](#-설치-1분이면-끝) · [동작 방식](#-동작-방식) · [명령 레퍼런스](#-명령-레퍼런스) · [예시](#-예시-도메인별-동적-전문가) · [문서](./CLAUDE.md)
...
2. **이 프로젝트에만 필요한** 전문가들을 직접 생성합니다
   - IoT/텔레메트리 프로젝트 → `mqtt-streaming-engineer`, `mongodb-changestream-specialist`
...
• 이름: my-shop
• 도메인: e-commerce
• 주 스택: Next.js, NestJS, PostgreSQL, Redis, Stripe
• 핵심 우려: 결제 안정성, 재고 동시성, 주문 상태 일관성

🏢 제안 전문가 (6명, + chronicler 공통)

1. domain-architect (Opus)
   📌 모듈 경계와 도메인 모델 설계
   🎯 주문/결제/재고 도메인 분리 필요

2. checkout-flow-engineer (Sonnet)
   📌 장바구니 → 결제 → 주문 확정 플로우 구현
   🎯 결제 중단·재시도·멱등성 처리가 핵심

3. payment-gateway-integrator (Opus)
   📌 Stripe 연동, 웹훅 검증, 환불 처리
   🎯 PCI 영역 분리와 멱등성 키 관리가 최우선
...
🏢 추천 팀 구성 (회사 6명 중 4명 + chronicler)
• domain-architect (Opus)              — 쿠폰 도메인 설계
• checkout-flow-engineer (Sonnet)      — 결제 플로우 통합
• inventory-pricing-dba (Opus)         — 스키마·동시성
• storefront-ui-engineer (Sonnet)      — 쿠폰 적용 UI
• chronicler (Haiku)                   — 기록
...
같은 플러그인이지만, **프로젝트의 도메인·스택에 따라 다른 전문가**가 생성됩니다.

| 프로젝트 도메인 | 생성될 수 있는 전문가 (예) |
|--------------|--------------------------|
| IoT / 텔레메트리 | `nestjs-modular-architect`, `mqtt-streaming-engineer`, `mongodb-changestream-specialist`, `timeseries-dba`, `grafana-observability-engineer`, `iot-security-reviewer`, `device-simulator-test-engineer` |
